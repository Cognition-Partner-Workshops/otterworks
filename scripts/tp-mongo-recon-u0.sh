#!/usr/bin/env bash
# One harness run over all four U0 collections; result.json is the only merge authority.
set -euo pipefail
...
"$PYTHON" "$REPO_ROOT/scripts/tp_mongo/unit_mapping.py" \
  --out "$OUT/mapping/u0.json" \
  --collection codes --collection tenants \
  --collection plans --collection fixture_meta
