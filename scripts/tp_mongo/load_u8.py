"""Build the U8 Tier-4 replay clone: `replay_u8_*` copies of the merged U5 billing set
plus the U0 `plans`/`tenants` references, inside ow_tp_mongodb_205236.

The invoicing module is replayed against this clone so the shared U5/U0 collections
are never mutated. Each clone collection is dropped and recreated from its source
collection on every run (`$out`), then re-indexed like the source; only
`replay_u8_*` names are written.
"""
...
def assert_owned(name: str) -> None:
    if not name.startswith(PREFIX):
        raise ValueError(f"refusing to write a collection U8 does not own: {name!r}")
...
def clone_collection(database: Any, source: str) -> dict[str, Any]:
    ...
    if source not in database.list_collection_names():
        raise RuntimeError(f"source collection {source!r} is missing; U8 depends on U5/U0")
