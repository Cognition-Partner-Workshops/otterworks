#!/usr/bin/env bash
# One harness run over the U7 billing audit log; result.json is the only merge authority.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUT="$REPO_ROOT/.migration/recon/U7"
cd "$REPO_ROOT"
