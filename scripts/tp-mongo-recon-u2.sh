#!/usr/bin/env bash
# One harness run over the U2 invoice feed; result.json is the only merge authority.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUT="$REPO_ROOT/.migration/recon/U2"
cd "$REPO_ROOT"
...
"$PYTHON" "$REPO_ROOT/scripts/tp_mongo/unit_mapping.py" \
  --out "$OUT/mapping/u2.json" \
  --collection invoice_feed
