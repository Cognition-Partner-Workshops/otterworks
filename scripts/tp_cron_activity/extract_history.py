...
def summary_envelope(ns: str, row: dict) -> dict:
    return {
...
def parse_history(ns: str, day: date, key: str, raw: bytes) -> list[dict]:
    try:
        text = gzip.decompress(raw).decode("utf-8")
    except (UnicodeDecodeError, OSError) as exc:
        return [{
            "kind": "top_user", "namespace": ns, "report_date": str(day),
...
def extract(ns: str, ds: str) -> tuple[list[dict], list[dict], list[str]]:
    ...
    with psycopg2.connect(**pg_kwargs()) as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            summaries = [summary_envelope(ns, row) for row in cur.fetchall()]
    s3, _, _ = clients()
    top_users = []
    missing = []
    for day in sorted(wanted):
        ...
        try:
            ...
            continue
        top_users.extend(parse_history(ns, day, key, body))
