#!/usr/bin/env python3
"""Wave 2 unit `dunning-ops`: DUNNING_ATTEMPTS + NOTIFICATIONS +
BILLING_AUDIT_LOG -> dunningAttempts / notifications / billingAuditLog on
the offline estate. billingAuditLog gets a TTL index on loggedAt
(expireAfterSeconds=7776000) replacing JOB_PURGE_AUDIT_LOG; numeric logId
stays the _id.

Reads Oracle via ORACLE_FIXTURE_DSN (JSON {"user","password","dsn"}) and
writes mongodb db `ow_billing_offline` via MONGO_LOCAL_URI. Refuses to run if
...
TTL_SECONDS = 7776000  # 90 days; replaces JOB_PURGE_AUDIT_LOG (04_jobs.sql:19-27)
