...
def _seed(conn):
    ...
    # AFTER customer_master: trg_customer_master_hist copies each deleted row
    # into HIST, so clearing HIST must come after the customer delete. The
    # hist_id band also clears seeded rows whose cust_id is NULL (sparse).
    cur.execute("DELETE FROM customer_master_hist WHERE cust_id LIKE 'SYNTH-CUST-%' OR hist_id >= 9100000")
