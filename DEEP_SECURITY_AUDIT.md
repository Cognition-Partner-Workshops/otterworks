...
4. **F-001 — file-service no longer trusts request-supplied identity.** Every identity-bearing handler now derives the user **only** from the gateway-injected `X-User-ID`: `upload_file` (multipart `owner_id` removed), `list_files`/`list_trashed`/`list_folders` (query `owner_id` removed and now **fail closed** when the header is absent instead of returning every user's records), and `create_folder` (body `owner_id` removed) / `share_file` (body `shared_by` removed).
...
- **F-002:** `go build ./...` and `go test ./internal/config/` pass. The new `TestValidate` asserts `Validate()` errors on the empty and default secrets without the allow-flag, and passes with the flag / a real secret. Confirmed the Helm chart does **not** set `ALLOW_INSECURE_JWT_SECRET`, so production fails closed.
- **F-004:** Re-checked `verify_chaos_secret` — in `production` with no secret the request is now rejected (`401`); `development`/`test` behavior unchanged. `ruby -c` passes.
- **F-005:** Re-checked `verify_alert_secret` — constant-time `secure_compare` in use, fails closed in non-dev when unset, and no committed `demo-alert-secret` remains in `docker-compose.yml`. Behavior under `RAILS_ENV=test` is unchanged, so the existing rspec suite is unaffected.
- **F-001:** `cargo fmt --check`, `cargo clippy -- -D warnings`, and `cargo test` (11 passed) all green. Re-read every file-service handler — owner/sharer identity is sourced solely from `X-User-ID`; supplying `owner_id` (multipart, query, or folder body) or `shared_by` (share body) no longer changes the resolved identity. The list handlers (`list_files`, `list_trashed`, `list_folders`) now return `400` when `X-User-ID` is missing instead of silently performing an unscoped query that returns all users' records (regression caught in PR review and fixed).
- **F-006:** Confirmed the wildcard and the Next.js auth-bypass entries are gone from `.trivyignore`; the next scan will report them.

> **Pre-existing scanner CI (`sast`, `secret-scan`, `dependency-scan`) failures are not caused by this PR.** They belong to `security-scan.yml` and are red on `main`: `sast` (semgrep) reports 21 findings in untouched files (`router.go`, `indexer.py`, `validate.py`, `auth.service.ts`); `secret-scan` (gitleaks) finds secrets committed in fixtures (`auth.service.ts`, `export-utils.ts`, a surefire test report); `dependency-scan` (trivy) reports CVEs in existing lockfiles. None of the un-suppressed CVEs (`CVE-2025-29927`, `CVE-2021-*`) appear in the trivy results, and no flagged file is part of this diff.
