#!/usr/bin/env python3
"""Deploy the cnvingest ingest unit to the shared demo Databricks workspace.

Imports the notebook + core module under /Shared/ow_tp/cnvingest/ and
creates (or resets) the ow_tp_ingest_cnvingest job from
job_ow_tp_ingest_cnvingest.json. Live execution belongs to the parent
...
WORKSPACE_DIR = "/Shared/ow_tp/cnvingest"
JOB_NAME = "ow_tp_ingest_cnvingest"
...
def main() -> int:
    ...
    import_source(
        dbx,
        HERE / "sftp_ingest_poll_notebook.py",
        f"{WORKSPACE_DIR}/sftp_ingest_poll_notebook",
        "SOURCE",
    )
    import_source(
        dbx,
        HERE / "ingest_core.py",
        f"{WORKSPACE_DIR}/ingest_core.py",
        "AUTO",
    )
    spec = json.loads((HERE / "job_ow_tp_ingest_cnvingest.json").read_text())
    existing = [
        job
