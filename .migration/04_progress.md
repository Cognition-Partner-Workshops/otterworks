...
| 2 | U7 PKG_RATING | RECON_GREEN | GREEN — fixture PASS (`.migration/recon/U7/result.json`: T1 5/5, T2 23/23, T3 892/892 full diff incl. 3 embedded `results`, T4 8/8 rating transcripts replayed on `replay_u7_*`; run 1 FAIL kept as `result.run1.json`: T1 `replay_u7_billing_audit_log` 1 doc vs 0 rows = the U5 golden's observer-caused audit row absent from a fresh fixture) | 0% (no quarantine targets declared; expected 0, observed 0) | LIVE recon gate (parent); `billing_audit_log` ungraded T1–T3 for U7 (write-only sink, U5-graded pre-state); Tier 4 source side = recorded Oracle transcripts (source SHA verified), not live PL/SQL; HTTP routes `/api/rating/*` not rewired (row 43, U6 calibration); suspension proration (status 20) + NULL-plan propagation + `DUP_VAL_ON_INDEX` on `id` unit-tested only (no fixture rows) | 3 loads + 2 recon runs (child) | — |
