...
| 2 | U6 PKG_OW_UTIL+PKG_PLANS (calibration) | RECON_GREEN | GREEN — FIXTURE PASS (T1 14/14, T2 67/67, T3 1005/1005, T4 5/5; mapping v1.0.1; tolerances v1; `.migration/recon/U6/result.json`; report validated; target counts and replay index names recomputed from Atlas at 2026-09-02 01:33 UTC) | 0% (no quarantine targets declared; all 12 replay collections matched expected counts) | LIVE-mode recon gate (parent); Golden (unprefixed) collections never written by U6: app routes exercised only against replay_u6_* (OW_BILLING_COLLECTION_PREFIX); golden `counters` seeds for seq_billing_audit_log / seq_subscriptions_hist remain U1's responsibility; Flask blueprint HTTP layer exercised by unit tests with fakes only; Tier-4 parity ran through call_entrypoint, not HTTP; f_str2dt accepts only the strict DD-MON-YY shape; Oracle TO_DATE leniency (other separators, 4-digit years) declared-unexercised; Concurrent plan changes (findOneAndUpdate write conflict / transaction retry path) not exercised; f_code_desc: no PKG_PLANS caller; unit-tested only | 4 loads + 2 fixture recon runs; report generated 2026-09-02 01:33 UTC | pending |
