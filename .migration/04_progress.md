...
| 3 | U6 dunning | ow.notifications, ow.invoices.dunning_attempts[] (coordinated additive embed, U5/U6 sequential batch) | MERGED | GREEN (independent LIVE recon PASS — gate Tier1 4/4, Tier2 10/10, Tier3 full diff with `invoices.dunning_attempts` embed graded, no exclusion, 0 findings; replay 20/20 fn_overdue_accounts LIVE cases + 5/5 recorded Oracle transcripts; mapping 1.2 / tolerances 1.0 — wave report `recon/wave3b-independent-20260901`:.migration/recon/wave_reports/wave3b.md) | 0% (0 rejects, quarantine unused, 0 orphan attempts) | fixture-only run; real PL/SQL not invoked (transcript replay only); disabled nightly scheduler replaced by a manual entrypoint, no schedule activated; `WHEN OTHERS THEN NULL` swallow and Atlas transaction rollback not induced (mongomock shim only); runtime writes to U0/U3-owned `tenants`/`subscriptions`/`subscriptions_hist` exercised only in replay; explicit `dunning_attempts: []` is a load invariant over migrated invoices — normalizing it on post-cutover invoices is a declared cross-unit follow-up in U5's `invoicing_service`; `UNKNOWN` code branches unexercised; suppressed audit-log write (U7 owns `billing_audit_log`) | M0 free tier, 1 notification + 1 embedded attempt across 3 invoices | #1412 |
...
also writes `ow.tenants` (U0) and `ow.subscriptions` plus `ow.subscriptions_hist` (U3)
at runtime — the history write ports the row-level `TRG_SUBSCRIPTIONS_HIST` that the
source UPDATE fires, reusing U3's `plans_service.history_doc`. That is migrated
...
Wave 3b independent recon report branch: `recon/wave3b-independent-20260901` (commit a88b731c, `.migration/recon/wave_reports/wave3b.md`).
