...
| 1 | U1 customers (XL) | RECON_GREEN | GREEN — fixture PASS (`.migration/recon/U1/result.json`: T1 3/3, T2 313/313, T3 33,333/33,333 full diff incl. 8,333 embedded `attributes`; `customers_history` graded empty; 0 warnings) | 0.324% (50 `dirty_signup_dt` + 31 `bad_csv_list` of 25,000; rows retained verbatim, twins null) | LIVE gate (parent); Tier 4 app write path (`customer_writes.py` — scratch-customer smoke only, `write_path_smoke.json`); mapping `derived_ungraded` twins/addresses/phones (unit tests only); RPT-114 route not run under gunicorn (test client + `rpt114_balances.json`); `counters` graded vs `USER_SEQUENCES` outside harness; harness sampling path (full diff) | 2 recon runs (run 1 red: full-mapping run touched non-U1 `DOCUMENTS`; run 2 on U1 projection green), 3 loads (~3 min each) | (this PR) |
