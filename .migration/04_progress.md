...
| 1 | U2 invoices | RECON_GREEN | GREEN — fixture PASS (`.migration/recon/U2/result.json`: T1 2/2, T2 9/9, T3 168,713/168,713 full diff incl. 149,963 embedded lines graded) | 0.197% (37/18,750 → Q.`invoice_feed_orphan_lines`; 0.025% of unit rows) | LIVE gate (parent); Tier 4 (none in contract; RPT-114 month-end checked by `scripts/tp_mongo/rpt114_parity_u2.py`, exact match); harness sampling path (full_diff only); derived `invoice_date`/`due_date`/`lines[].gl_accounts` (derived_ungraded, loader unit tests only); admin-dashboard UI not exercised | 2 loads + 1 recon (green first run), ~5 min | (this PR) |
