...
Resume note (wave-1, run wfr-cc00292593f54696bc43d58b68bedd34): the first pass exposed a plan defect —
briefs pointed children at the whole 6-collection mapping and the harness has no collection
filter, so a child's verdict depended on sibling batches (w1-b01 BLOCKED, w1-b02 FAIL on sibling
collections only). Fix (grading-only, STOP A P3): `.migration/mappings/<batch>.json` and
`<batch>.fixture.json`, mechanical subsets generated by `waves/build_wave1.py`; briefs for
w1-b01/w1-b02 updated. Briefs for the merged batches w1-b03..b05 are kept byte-identical so the
workflow replays them instead of relaunching.
