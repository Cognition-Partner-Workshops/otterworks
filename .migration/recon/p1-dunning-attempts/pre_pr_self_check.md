# Pre-PR self-check - p1-dunning-attempts
...
|---|---|---|
| NULL / missing attribution cannot fail open | pass | every column of `billing.dunning_attempts` is `NOT NULL`; the scheduler derives `attempt_no` from `COALESCE(MAX(attempt_no), 0) + 1`, so a missing history attributes to attempt 1 rather than NULL |
...
| no DDL drops / replaces / alters a shared table | pass | `w3d_dunning_attempts.sql` is `CREATE TABLE IF NOT EXISTS` only; no other unit's table is touched |
...
| cleanup paths retain run evidence | pass | evidence lives under `.migration/recon/p1-dunning-attempts/` and is committed, not cleaned |
...
| every unverified path listed | pass | `unverified_paths` in `p1-dunning-attempts.recon.json` and the "unverified" section of `summary.md` |
| machine-readable report present | pass | `.migration/recon/p1-dunning-attempts/p1-dunning-attempts.recon.json`, `"kind": "recon-report"`, `make tp-validate-recon` green |
