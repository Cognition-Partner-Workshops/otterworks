...
set -uo pipefail
N=$1; RESET=${2:-}
source "$HOME/cutover_work/pr2/env.sh"
REPO="$HOME/cutover_work/otterworks"
C="$HOME/cutover_work/pr2/cycle$N"; mkdir -p "$C"
...
step source_pre   $PY "$HOME/cutover_work/pr2/source_check.py" "$C/source_pre.json"
if [ "$RESET" = "reset" ]; then
...
for U in U0 U1 U2 U5; do
  mkdir -p "$C/$U"
  $PY "$HOME/cutover_work/pr2/subset.py" "$MAP" $U "$C/$U/mapping_${U,,}_subset.json" > "$C/$U/subset.txt"
...
step guards       $PY "$HOME/cutover_work/pr2/guards.py" "$C"
step source_post  $PY "$HOME/cutover_work/pr2/source_check.py" "$C/source_post.json"
