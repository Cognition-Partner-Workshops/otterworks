...
pre = json.load(open(os.path.expanduser("~/wave_recon/w1b/pre_state.json")))
...
man = json.load(open(os.path.expanduser("~/wave_recon/w1b/manifest_demo.json")))
...
W01 = {"codes", "tenants", "plans", "counters", "customers", "customers_history", "invoices", "documents", "document_snapshots", "files"}
extra = set(db.list_collection_names()) - W01
ok("target.collections_registry_wave01_present_and_no_u2_residue", W01 <= set(db.list_collection_names()) and not [c for c in extra if c.startswith("invoices")], f"wave0/1 present; later-wave collections (U5-U9, expected): {sorted(extra)}")
