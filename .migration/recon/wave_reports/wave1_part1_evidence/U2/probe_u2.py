...
results.append({"probe": "11.3 loader drops live `invoices` BEFORE reinsert (no staging swap; empty-batch would leave an EMPTY collection, and concurrent readers see a gap) (design note)", "ok": True, "detail": f"drop_before_insert={src}"}); print("info 11.3 U2 loader drop-before-insert, no empty-batch guard:", src)
hd = int(q("SELECT COUNT(*) FROM invoice_header WHERE batch_no = 1")[0][0])
ok("11.4 an empty batch (batch_no=1) has 0 headers/0 lines in source -> the loader WOULD load 0 docs (no refusal guard, unlike U1) — documented, not exercised", hd == 0, "not exercised against the live target")
