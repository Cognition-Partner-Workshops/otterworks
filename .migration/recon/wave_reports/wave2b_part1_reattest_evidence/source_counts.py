...
import hashlib
out["pkg_body_sha256"]={}
for n in ('PKG_OW_UTIL','PKG_PLANS','PKG_RATING'):
...
cur.execute("select object_name, status from user_objects where object_name in ('PKG_OW_UTIL','PKG_PLANS','PKG_RATING') and object_type='PACKAGE BODY'"); out["pkg_status"]=dict(cur.fetchall())
