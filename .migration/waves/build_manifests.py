...
def brief(wave, bid, units):
    ...
    colls = ','.join(c for u in units for c in UNITS[u]['colls'])
    lines.append("Fixture bootstrap on your own machine (you have no shared fixture; build it once, reuse it): from the repo root run `make oracle-billing-up && make oracle-billing-seed NS=demo` (Oracle Free container, ~5 min; mechanics in .agents/skills/oracle-billing-estate/SKILL.md, which also documents the demo schema user and how to find the mapped host port) and `docker run -d --name ow-mongo -p 27017:27017 mongo:7`. Then set, in your shell only, `MONGO_LOCAL_URI=mongodb://localhost:27017` and `OW_BILLING_FIXTURE_DSN` as the JSON {\"user\",\"password\",\"dsn\"} for the demo schema (values come from that skill/Makefile; never write them into files, PRs or logs). Recon venv: if `~/.venvs/recon/bin/recon selftest` fails, run `python3 -m venv ~/.venvs/recon && ~/.venvs/recon/bin/pip install -e \"$(ls -d /opt/.devin/plugins/cache/*mongo-migration-plugin*/*/skills/mongo-recon-harness/harness | head -1)[all,test]\"` first. Loader code lives under services/legacy-billing/migration/mongo/ (see services/legacy-billing/migration/mongo/load_codes.py on branch tp-run/mongodb-20260926T164803Z-rt-offline--w0-b01, PR #1708 — unmerged, so it is NOT on the run branch — for the accepted pattern: allowlist check, secrets by env-var name, drop+recreate own collections only, deterministic _id, quarantine with reason codes). Commit recon evidence (result.json, recon.summary.md, report.md) under services/legacy-billing/migration/mongo/recon/<unit>/ on your branch.")
    lines.append(f"Recon (from the repo root): `env -u MONGODB_ATLAS_URI ~/.venvs/recon/bin/recon run --unit <unit_id> --family oracle --mapping <spec> --tolerances .migration/02_tolerances.json --canonicalization $(ls -d /opt/.devin/plugins/cache/*mongo-migration-plugin*/*/skills/mongo-migration/profiles/oracle.md | head -1) --mode fixture --target-class local --source-dsn-secret OW_BILLING_FIXTURE_DSN --target-uri-secret MONGO_LOCAL_URI --target-db ow_billing_migration --allowed-targets-file .migration/allowed_targets.json --source-concurrency 1 --seed 1 --out /tmp/recon/<unit_id>/`. The harness has no per-collection flag: <spec> is a verbatim subset copy of .migration/03_mapping_spec.json (same version + canonicalization block) containing only your collections ({colls or 'none: code unit — run the procs/harness parity transcripts against the migrated service instead and commit the transcripts'}); never edit the spec itself.")
    lines.append("Lessons folded in from wave 1 (skill feedback): write generated SQL as an inline literal/f-string inside cursor.execute(...) (the co-installed dbx guard rejects execute(<variable>)); reuse ONE Oracle connection for loader and parity replay (per-call connections exhaust the Oracle Free listener; source concurrency is 1); new Python deps (pymongo, oracledb) must be added to the `uv run --with ...` lists in both the legacy-billing Makefile and .github/workflows/tp-golden-smoke.yml, or imported lazily; fixture counts include static rows from schema/04_upgrade_static.sql (e.g. 25001 customers, not 25000) — trust the fixture manifest's row_counts; Mongo read paths must render Decimal128 via normalize() and Int64 via str() to stay byte-identical to the Oracle facade JSON; the harness salts keys in findings, so prove quarantine<->diff correspondence at collection/field/count level; `make tp-smoke` Go/Node stages may not run on your box — rely on CI for those.")
    lines.append("Playbook text if `!mongo_unit_migration` does not resolve as a playbook: $(ls -d /opt/.devin/plugins/cache/*mongo-migration-plugin*/*/skills/install-mongo-kit/playbooks/3-unit_migration.md | head -1).")
