...
| Snapshots | `etl/legacy-extra/commission_dw/<ns>/` holds extract CSVs + `manifest.json`. Two distinct steps: (1) **local transport check** — `FIXTURE_SOURCE=etl/legacy-extra/commission_dw/<ns> make tp-fixture-land NS=<ns>` + `make tp-fixture-verify NS=<ns>` (writes only under `.tp-preflight/`, proves byte/checksum integrity, used by children for `run_mode: fixture`); (2) **Databricks bronze landing** — parent-owned upload of the same bytes via the Files API (`PUT /api/2.0/fs/files/Volumes/ow_tp/bronze/landing/<ns>/<unit>/<file>`, using `DATABRICKS_DEMO_TOKEN`) followed by a sha256 re-read check against `manifest.json`; target loads read only from the volume path. |
