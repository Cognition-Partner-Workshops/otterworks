...
| STOPB-FRAMEWORK | Pipeline framework is Databricks Workflows + notebook tasks with Delta `MERGE`, not Lakeflow Declarative Pipelines and not dbt. One unit = one job `ow_tp_<unit>`; restart safety comes from `MERGE` on the deterministic `f_md5_uuid` key plus `ns`; quarantine and recon accounting are written explicitly rather than inherited from expectations. | Customer | 2026-08-28 | Customer STOP B decision; `.migration/08_unit_inventory.md`, `.migration/10_wave_plan.md` |
