...
|---|---|---|---|---|---|---|---|
| U1 `dim_agent` | `01_star_schema.sql` (DDL); `02_etl_pkg.sql` L23–33 (MERGE) | SQL + PIPELINE step | `COMMISSION_PAY.AGENTS` | `ow_tp.silver.dim_agent_cdw` | S | no | identity key preservation: table initialised from the `DIM_AGENT` baseline snapshot with its `agent_key` values; new agents get `max(agent_key)+row_number()`; MERGE updates 3 attrs |
...
| U4 `fact_commission` + loader job | DDL; L59–87; whole procedure as the job | PIPELINE (money path) | ledger ⋈ policies ⋈ dim_agent ⋈ dim_product ⋈ dim_period | `ow_tp.silver.fact_commission_cdw`; job `ow_tp_cdw_load_commission_facts` (tasks: T0 ingest feed — validate manifest, replace the 4 `ow_tp.bronze.<feed>_cdw` tables; then U1→U2→U3→U4 MERGEs, then U5 rebuild) | M | no | **inner joins drop ledger rows with unknown agent/product/period silently**; WHEN MATCHED does **not** update `product_key`; `loaded_at` server-side; `SQL%ROWCOUNT` = fact MERGE rows only; NUMBER(12,2) money; transaction atomicity (all-or-nothing) has no Delta equivalent across 4 tables |
...
| U3 | rowcount; row diff on `period_key`; derived cols recomputed from `period_month`; **key-preservation check**: every baseline key present with identical natural key (applies to U1, U2, U4 too) | `period_key` / `period_month` | full | same (insert-only semantics preserved) |
