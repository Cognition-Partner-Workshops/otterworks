...
| 2026-09-18 | App backend parity | 51 requests (all 9 app tenants, 3 invoices, /plans, overdue at two dates) byte-identical between the PostgreSQL routes and the mongo backend (PR #1656); index page differs only in the Backend line; write routes answer 501 on mongo | playbook 5 step 2 | FACT |
| 2026-09-18 | STOP C (work branch) | APPROVE, for the work branch only: flip the app's default BILLING_BACKEND to mongo on tp-run/mongodb-20260918T212022Z, show rollback by BILLING_BACKEND=postgres, flip back, merge after smoke | parent approver | FACT |
| 2026-09-18 | STOP C (production) | OPEN. No live recon (no source access) and no cutover principal. Production cutover needs the customer's in-network LIVE or SNAPSHOT recon and their own cutover principal; nothing in this run substitutes for either | parent approver | FACT |
