...
def load_table(table: dict, ora, pg, schema: str) -> dict:
    ...
    with pg.cursor() as cur:
        ...
        # application insert and collide with a migrated row. Move it past the data - and
        # only ever forwards: an Oracle sequence does not rewind when its top rows are
        # deleted, so neither may this one, or a rerun would reissue keys the source has
        # already handed out. Non-identity keys have no sequence and are left alone.
        for col in key_cols:
            ...
            (from_data,) = cur.execute(
                f"SELECT coalesce(max({col}), 0) + 1 FROM {target}").fetchone()
