"""Content fingerprint of the p3-storage-cleanup tables for one batch.

Row counts plus an order-independent md5 over each table's whole rows, so an idempotency
rerun is compared on contents: a recompute that swapped one candidate for another of the
same size would keep the count and move the digest.

`computed_at` is excluded. It is a clock reading, not output: including it would make every
rerun differ and prove nothing.

    python3 fingerprint_cleanup_target.py p3probe idempotency.before.json
"""
import json
import sys
...
TABLES = (
    ("bronze.p3_cleanup_inventory_raw", "snapshot_batch, object_key, size_bytes", True),
    ("bronze.p3_cleanup_metadata_raw",
     "snapshot_batch, file_id, s3_key, owner_id, size_bytes", True),
    ("silver.storage_cleanup_candidates",
     "snapshot_batch, run_date, object_key, size_bytes, quarantine_key", True),
    ("bronze.p3_cleanup_legacy_delete_set", None, False),
)
