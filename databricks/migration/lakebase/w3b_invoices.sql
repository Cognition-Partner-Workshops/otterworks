...
--   enforced by nothing - same as Oracle); TIMESTAMP -> timestamp (no zone), UTC assumed
--   and declared (P1-D3).
--
-- Derived dialect rule, carried in the PR body and the report: the mapping spec writes
-- `timestamptz` for issued_at, but Oracle TIMESTAMP is zoneless and the harness compares
-- TIMESTAMP(n>3) under the `identity` canonicalization rule, so a timestamptz target comes
-- back tz-aware and every row is a Tier-3 field_diff against the naive source value. The
-- same correction was already made in wave 0 (w0a_pkg_ow_util.sql:127, billing_audit_log
-- created_at timestamptz -> logged_at timestamp(0)). The column is therefore `timestamp`;
-- the instant is unchanged and UTC stays the declared assumption.
--
...
-- An earlier run of this script on the wave branch created issued_at as timestamptz (the
-- mapping spec's literal target_type, before the recon run above showed why it cannot be
-- one). Bring an already-created table onto the zoneless type in place; the stored
-- instants are UTC, so the cast changes no value.
DO $$
BEGIN
...
COMMENT ON COLUMN billing.invoices.issued_at IS
    'Oracle TIMESTAMP, zoneless on both sides; UTC assumed and declared (P1-D3).';
