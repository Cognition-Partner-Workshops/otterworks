...
public class ReportRequestValidator : AbstractValidator<ReportRequest>
{
    public ReportRequestValidator()
    {
        ...
        RuleFor(x => x.Category).NotNull().WithMessage("Report category is required");
        RuleFor(x => x.ReportType).NotNull().WithMessage("Report type is required");
        RuleFor(x => x.RequestedBy).NotEmpty().WithMessage("Requester ID is required");
    }
}
