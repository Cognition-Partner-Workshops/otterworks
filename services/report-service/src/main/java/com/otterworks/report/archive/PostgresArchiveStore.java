...
/**
 * Reads the migrated {@code arch.*} tables on the tenant's PostgreSQL database.
 *
 * TIMESTAMP(12) text is rebuilt from {@code TIMESTAMP(6)} (rendered server-side with
 * {@code TO_CHAR} so the driver never rounds) plus the six-digit {@code _NANOS_TAIL}
 * (CONTRACTS.md §6.0). Column names are quoted because the DDL keeps Db2's upper-case spelling.
 */
public class PostgresArchiveStore extends JdbcArchiveStore {

    private static final String TS = "YYYY-MM-DD HH24:MI:SS.US";
    ...
    static final String EVENTS_SQL =
            ...
            + "FROM arch.\"FILEAUD\" WHERE \"ARCH_KEY\" = RTRIM(?) "
    ...
    static final String POLICY_SQL =
            ...
            + "\"EFFECTIVE_TS_NANOS_TAIL\" FROM arch.\"RETNPLCY\" WHERE RTRIM(\"POLICY_CODE\") = RTRIM(?)";

    static final String PING_SQL = "SELECT 1";

    public PostgresArchiveStore(JdbcTemplate jdbc) {
        super(jdbc);
    }

    @Override
    public String storeName() {
        return ArchiveStoreType.POSTGRESQL.wireName();
    }
    ...
    static ArchiveVersion mapVersion(ResultSet rs) throws SQLException {
        ...
        v.lastAccessTs = Db2Text.timestamp12FromPgTimestamp(
    ...
    static ArchiveEvent mapEvent(ResultSet rs) throws SQLException {
        ...
        e.eventTs = Db2Text.timestamp12FromPgTimestamp(rs.getString("EVENT_TS"), rs.getInt("EVENT_TS_NANOS_TAIL"));
