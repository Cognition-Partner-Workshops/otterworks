...
pub async fn upload_file(
    ...
    // CHAOS: when chaos injection is enabled (CHAOS_ENABLED=true) and this
    // flag is active the S3 client targets a nonexistent bucket, simulating a
    // misconfigured bucket name after a recent infra change.  The AWS SDK
    // returns NoSuchBucket which surfaces as a 500.
    let effective_bucket = if config.server.chaos_enabled
        && chaos_active(
