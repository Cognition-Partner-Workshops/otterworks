...
def user_headers(tenant_id: UUID) -> dict[str, str]:
    return {"X-User-ID": "user-001", "X-Tenant-ID": str(tenant_id)}
...
def test_user_cannot_read_or_change_another_tenant(client: TestClient) -> None:
    ...
    write = client.post(
        f"/api/tenants/{TENANT_B}/plan-change", json=PAYLOAD, headers=user_headers(TENANT_A)
    )
...
def test_user_can_act_on_own_tenant(client: TestClient) -> None:
    ...
    write = client.post(
        f"/api/tenants/{TENANT_A}/plan-change", json=PAYLOAD, headers=user_headers(TENANT_A)
    )
...
def test_incomplete_identity_headers_are_rejected(client: TestClient) -> None:
    missing_tenant = client.get(
        f"/api/tenants/{TENANT_A}/entitlement",
        params={"on": "2026-02-28"},
        headers={"X-User-ID": "user-001"},
    )
    malformed_tenant = client.get(
        f"/api/tenants/{TENANT_A}/entitlement",
        params={"on": "2026-02-28"},
        headers={"X-User-ID": "user-001", "X-Tenant-ID": "not-a-uuid"},
    )

    assert missing_tenant.status_code == 401
    assert malformed_tenant.status_code == 401
...
def test_wrong_service_token_is_rejected(client: TestClient) -> None:
    response = client.get(
        f"/api/tenants/{TENANT_A}/entitlement",
        params={"on": "2026-02-28"},
        headers={"Authorization": "Bearer wrong-token", **user_headers(TENANT_A)},
    )
