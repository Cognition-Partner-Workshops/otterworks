...
class SqsConsumerTest {
    ...
    @Test
    fun `parseMessage handles legacy epoch integer timestamp`() {
        val body = """
            {
                "eventType": "file_shared",
                "fileId": "file-legacy",
        ...
        assertEquals("2024-06-27T06:33:20Z", event.timestamp)
    }

    @Test
    fun `parseMessage handles epoch timestamp in SNS-wrapped message`() {
        val innerMessage = """{"eventType":"comment_added","userId":"user-1","actorId":"actor-1","documentId":"doc-1","commentId":"c-1","timestamp":1719470000}"""
        val escapedInner = innerMessage.replace("\"", "\\\"")
        val body = """
            {
                "Type": "Notification",
    ...
    @Test
    fun `parseMessage handles message with unknown fields`() {
        val body = """
            {
                "eventType": "file_shared",
                "fileId": "file-123",
