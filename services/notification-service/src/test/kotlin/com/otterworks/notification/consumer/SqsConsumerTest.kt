...
class SqsConsumerTest {
    ...
    @Test
    fun `parseMessage parses Unix epoch integer timestamp`() {
        val body = """
            {
                "eventType": "file_shared",
                "fileId": "file-legacy",
    ...
    @Test
    fun `parseMessage parses Unix epoch millis timestamp`() {
        val body = """
            {
    ...
    @Test
    fun `parseMessage parses SNS-wrapped message with epoch timestamp`() {
        val innerMessage = """{"eventType":"file_shared","fileId":"f-1","ownerId":"o-1","sharedWithUserId":"u-2","timestamp":1704067200}"""
        val escapedInner = innerMessage.replace("\"", "\\\"")
        val body = """
            {
                "Type": "Notification",
