...
class SqsConsumerTest {
    ...
    fun `parseMessage handles legacy epoch integer timestamp`() {
        ...
        assertEquals("1718441400", event.timestamp)
    }

    @Test
    fun `parseMessage handles SNS-wrapped message with epoch timestamp`() {
        val innerMessage = """{"eventType":"comment_added","userId":"user-1","actorId":"actor-1","documentId":"doc-1","commentId":"c-1","timestamp":1718441400}"""
        val escapedInner = innerMessage.replace("\"", "\\\"")
        val body = """
            {
                "Type": "Notification",
    ...
    @Test
    fun `parseMessage handles unknown extra fields gracefully`() {
        val body = """
            {
                "eventType": "file_shared",
                "fileId": "file-123",
