...
class SqsConsumerTest {
    ...
    @Test
    fun `parseMessage handles legacy epoch timestamp`() {
        val body = """
            {
                "eventType": "file_shared",
                "fileId": "file-legacy",
    ...
    @Test
    fun `parseMessage handles legacy epoch timestamp in SNS envelope`() {
        val innerMessage = """{"eventType":"comment_added","userId":"user-1","actorId":"actor-1","documentId":"doc-1","commentId":"c-1","timestamp":1719480000}"""
        val escapedInner = innerMessage.replace("\"", "\\\"")
        val body = """
            {
                "Type": "Notification",
