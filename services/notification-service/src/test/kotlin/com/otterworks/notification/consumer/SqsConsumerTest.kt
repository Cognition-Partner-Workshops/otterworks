...
class SqsConsumerTest {
    ...
    @Test
    fun `parseMessage handles legacy Unix epoch timestamp`() {
        val body = """
            {
                "eventType": "file_shared",
                "fileId": "file-legacy",
    ...
    fun `parseMessage handles SNS-wrapped message with epoch timestamp`() {
        val innerMessage = """{"eventType":"comment_added","userId":"user-1","actorId":"actor-1","documentId":"doc-1","commentId":"c-1","timestamp":1718443800}"""
        val escapedInner = innerMessage.replace("\"", "\\\"")
        val body = """
            {
                "Type": "Notification",
