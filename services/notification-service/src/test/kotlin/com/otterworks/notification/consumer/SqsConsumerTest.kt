...
class SqsConsumerTest {
    ...
    @Test
    fun `parseMessage parses legacy epoch integer timestamp`() {
        val body = """
            {
                "eventType": "file_shared",
                "fileId": "file-legacy",
    ...
    fun `parseMessage parses SNS-wrapped message with epoch timestamp`() {
        val innerMessage = """{"eventType":"comment_added","userId":"user-1","actorId":"actor-1","documentId":"doc-1","commentId":"c-1","timestamp":1718445000}"""
        val escapedInner = innerMessage.replace("\"", "\\\"")
        val body = """
            {
                "Type": "Notification",
