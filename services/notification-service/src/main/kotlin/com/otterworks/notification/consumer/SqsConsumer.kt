                ...
                for (msg in messages) {
                    launch {
                        try {
                            ...
                            // successfully, or it is a poison message that would
                            // otherwise re-enter the queue after the visibility
                            // timeout and cause unbounded queue growth.
    ...
    internal fun parseMessage(body: String): SqsNotificationMessage? {
        ...
        return try {
            ...
            try {
                ...
                if (useStrict) {
                    // Strict parser failed; fall back to lenient parser to
                    // handle legacy messages with non-RFC-3339 timestamps.
                    parseLenient(body)
                } else {
