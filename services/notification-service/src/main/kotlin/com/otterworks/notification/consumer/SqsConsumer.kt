...
class SqsConsumer(
                    ...
                    launch {
                        try {
                            ...
                            if (event != null) {
                                ...
                                logger.error { "Permanently failed to parse SQS message, deleting to prevent re-delivery: ${msg.messageId}" }
                            }

                            // Always delete the message — both after successful
                            // processing and after a permanent parse failure —
    ...
    internal fun parseMessage(body: String): SqsNotificationMessage? {
        ...
        return try {
            ...
            try {
                ...
                // Strict parser failed — fall back to the lenient parser so
                // legacy messages (e.g. integer epoch timestamps) are still
                // processed instead of stuck in the queue forever.
                if (useStrict) {
                    logger.warn { "Strict parse failed, retrying with lenient parser" }
                    parseLenient(body)
                } else {
    ...
    private fun parseLenient(body: String): SqsNotificationMessage? {
        return try {
            ...
            try {
                ...
                logger.error(e) { "Lenient parse also failed" }
                null
            }
        }
    }
