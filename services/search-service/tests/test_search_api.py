...
class TestSuggestEndpoint:
    ...
    def test_suggest_without_ranking_score_does_not_error(self, client, mock_meilisearch_client):
        """Hits missing _rankingScore still produce suggestions instead of a 500."""
        mock_index = mock_meilisearch_client.index.return_value
        mock_index.search.return_value = {
            "estimatedTotalHits": 2,
            "hits": [
    ...
    def test_suggest_backend_failure_degrades_gracefully(self, client, mock_meilisearch_client):
        """A MeiliSearch failure yields an empty suggestion list, not a 500."""
        mock_index = mock_meilisearch_client.index.return_value
