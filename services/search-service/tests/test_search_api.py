...
class TestSuggestEndpoint:
    ...
    def test_suggest_orders_by_ranking_score(self, client, mock_meilisearch_client):
        """Suggestions are ordered by _rankingScore, highest first, and returned as strings."""
        mock_index = mock_meilisearch_client.index.return_value
        mock_index.search.return_value = {
    ...
    def test_suggest_hits_without_ranking_score_do_not_500(self, client, mock_meilisearch_client):
        """A hit missing _rankingScore (or with a bad type) still yields a 200 with string suggestions."""
        mock_index = mock_meilisearch_client.index.return_value
        mock_index.search.return_value = {
            "estimatedTotalHits": 3,
            "hits": [
        ...
        assert response.status_code == 200
        data = response.get_json()
        assert data["suggestions"] == ["Scored", "Unscored", "Bad"]
