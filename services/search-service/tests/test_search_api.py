...
class TestSuggestEndpoint:
    ...
    def test_suggest_ranking_enrichment_missing_score(
        self, client, mock_meilisearch_client, monkeypatch
    ):
        """Ranking enrichment must not 500 when _rankingScore is absent."""
    ...
    def test_suggest_ranking_enrichment_empty_results(
        self, client, mock_meilisearch_client, monkeypatch
    ):
        """Ranking enrichment with no hits returns an empty list, not 500."""
        monkeypatch.setattr("app.api.search._chaos_active", lambda key: True)
        mock_index = mock_meilisearch_client.index.return_value
