...
class TestSuggestEndpoint:
    ...
    def test_suggest_ranking_path_missing_ranking_score(self, client, mock_meilisearch_client, monkeypatch):
        """Ranking-enrichment path never 500s when _rankingScore is absent."""
    ...
    def test_suggest_ranking_path_empty_results(self, client, mock_meilisearch_client, monkeypatch):
        """Ranking-enrichment path with no results returns 200 and empty list."""
        monkeypatch.setattr("app.api.search._chaos_active", lambda key: True)
        mock_index = mock_meilisearch_client.index.return_value
