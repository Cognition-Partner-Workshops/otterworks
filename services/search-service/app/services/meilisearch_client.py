...
class MeiliSearchService:
    ...
    def suggest_ranked(self, prefix: str, size: int = 10) -> list[dict[str, Any]]:
        """Autocomplete suggestions with ranking scores for relevance sorting.

        Returns raw hit dicts containing at least ``title`` or ``name``
        and ``_rankingScore`` so callers can sort by relevance.
        """
