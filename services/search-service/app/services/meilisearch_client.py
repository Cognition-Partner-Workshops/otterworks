...
class MeiliSearchService:
    ...
    def suggest_ranked(self, prefix: str, size: int = 10) -> list[str]:
        """Autocomplete suggestions sorted by MeiliSearch ranking score."""
        ...
        ranked = sorted(hits, key=lambda h: h.get("_rankingScore", 0), reverse=True)
        return [
            h.get("title") or h.get("name", "") for h in ranked[:size]
        ]
