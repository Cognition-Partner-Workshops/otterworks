...
def suggest() -> tuple:
    ...
    # When the ranking-score enrichment flag is active, sort suggestions
    # by MeiliSearch _rankingScore for better relevance ordering.
    if _chaos_active("chaos:search-service:suggest_500"):
        try:
            service = _get_service()
            scored = service.suggest_with_scores(prefix)
            ranked = sorted(scored, key=lambda s: s.get("_rankingScore", 0.0), reverse=True)
            suggestions = [s["title"] for s in ranked]
            return jsonify({"suggestions": suggestions, "query": prefix}), 200
        except Exception:
            logger.exception("suggest_ranked_failed", prefix=prefix)
            return jsonify({"suggestions": [], "query": prefix}), 200
