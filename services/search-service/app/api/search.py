...
def suggest() -> tuple:
    ...
    # Ranking-score enrichment path: sort suggestions by relevance using
    # _rankingScore when available. Uses safe access to handle results that
    # may not include the ranking field.
    if _chaos_active("chaos:search-service:suggest_500"):
        ...
        ranked = sorted(
            raw_suggestions,
            key=lambda s: s.get("_rankingScore", 0) if isinstance(s, dict) else 0,
            reverse=True,
        )
