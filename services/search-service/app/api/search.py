...
def _ranking_score(suggestion: object) -> float:
    """Return the MeiliSearch ranking score for a suggestion, if present."""
...
def suggest() -> tuple:
    ...
    if _chaos_active("chaos:search-service:suggest_500"):
        try:
            ...
            # Sort by MeiliSearch ranking score when available; suggestions
            # without a score (plain strings or hits missing _rankingScore)
            # keep a neutral score instead of crashing the handler.
            ranked = sorted(raw_suggestions, key=_ranking_score, reverse=True)
