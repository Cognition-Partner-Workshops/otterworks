...
def suggest() -> tuple:
    ...
    try:
        ...
        if _chaos_active("chaos:search-service:suggest_500"):
            # Ranking-score enrichment: sort suggestions by MeiliSearch
            # relevance. _rankingScore is only present when the backend
            # returns enriched hits, so fall back to 0.0 when absent.
