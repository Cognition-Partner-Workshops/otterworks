...
def suggest() -> tuple:
    ...
    try:
        ...
        if _chaos_active("chaos:search-service:suggest_500"):
            # Ranking-score enrichment path: sort suggestions by relevance
            # using MeiliSearch's _rankingScore, requested explicitly via
            # showRankingScore so the field is always present.
            suggestions = service.suggest_ranked(prefix)
        else:
