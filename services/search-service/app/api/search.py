...
def suggest() -> tuple:
    ...
    # CHAOS: when the flag is active AND CHAOS_ENABLED=true, inject a
    # deliberate 500 to exercise alerting pipelines.
    if (
        os.getenv("CHAOS_ENABLED", "").lower() in ("1", "true")
        and _chaos_active("chaos:search-service:suggest_500")
    ):
        raise RuntimeError("chaos: search-service suggest_500 triggered")
