...
def suggest() -> tuple:
    ...
    # Ranking-score enrichment path: sort suggestions by _rankingScore when
    # results carry one. MeiliSearch only returns the field when explicitly
    # requested, so treat it as optional and never let its absence crash the
    # handler.
