...
def suggest() -> tuple:
    ...
    # MeiliSearch relevance score for better ordering.
    if _chaos_active("chaos:search-service:suggest_500"):
        service = _get_service()
        ranked_hits = service.suggest_ranked(prefix)
        ranked = sorted(ranked_hits, key=lambda s: s.get("_rankingScore", 0), reverse=True)
        suggestions = [hit.get("title") or hit.get("name", "") for hit in ranked]
