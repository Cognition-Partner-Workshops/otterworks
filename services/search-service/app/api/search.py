...
def suggest() -> tuple:
    ...
    try:
        ...
        if _chaos_active("chaos:search-service:suggest_500"):
            # Ranking-score enrichment: order suggestions by MeiliSearch's
            # _rankingScore when present, treating entries without a score
            # (or plain-string suggestions) as score 0.
            suggestions = sorted(
                suggestions,
