...
def suggest() -> tuple:
    ...
    use_ranking = _chaos_active("chaos:search-service:suggest_500")

    try:
        service = _get_service()
        if use_ranking:
            ranked_hits = service.suggest_ranked(prefix)
            ranked_hits.sort(key=lambda s: s.get("_rankingScore", 0), reverse=True)
            suggestions = [s["text"] for s in ranked_hits]
        else:
