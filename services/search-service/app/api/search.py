...
def _ranking_score(suggestion: object) -> float:
    """Safe ranking-score accessor: entries without a numeric _rankingScore rank last."""
...
def suggest() -> tuple:
    ...
    try:
        ...
        if _chaos_active("chaos:search-service:suggest_500"):
            # Ranking-score enrichment: order by MeiliSearch _rankingScore
            # when present; entries without a score keep a neutral rank.
