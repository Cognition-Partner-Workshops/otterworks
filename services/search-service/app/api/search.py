...
def suggest() -> tuple:
    ...
    # Relevance ordering by MeiliSearch _rankingScore is opt-in via this flag.
    ranked = _chaos_active("chaos:search-service:suggest_500")

    try:
        service = _get_service()
        suggestions = service.suggest(prefix, ranked=ranked)
