...
def suggest() -> tuple:
    ...
    # When the ranking-score enrichment chaos flag is active, attempt to
    # sort suggestions by MeiliSearch relevance.  service.suggest() returns
    # list[str], so ranking-score sorting is a no-op — we just return the
    # suggestions in the order MeiliSearch already ranked them.
