...
def suggest() -> tuple:
    ...
    # Ranking-score enrichment path: sorts suggestions by relevance using
    # MeiliSearch's _rankingScore. The field is only present when explicitly
    # requested, so treat it as optional to avoid crashing the handler.
