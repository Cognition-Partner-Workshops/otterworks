...
def suggest() -> tuple:
    ...
    # When the ranking-score enrichment flag is active, sort suggestions by
    # MeiliSearch relevance score for better autocomplete ordering.
    if _chaos_active("chaos:search-service:suggest_500"):
        service = _get_service()
        raw_hits = service.suggest_with_scores(prefix)
        ranked = sorted(raw_hits, key=lambda s: s.get("_rankingScore", 0), reverse=True)
        suggestions = [h.get("title") or h.get("name", "") for h in ranked]
