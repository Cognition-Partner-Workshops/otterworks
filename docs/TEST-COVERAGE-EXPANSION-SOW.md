...
## 9. WP-00 delivery notes (landed)

The coverage harness described in §5 is now in the repo. What a worker needs to know:

### The harness

- `scripts/coverage/run-unit.sh <unit>` runs one build unit's suite with coverage
  instrumentation and writes the machine-readable report plus a `status.txt` to
  `coverage-reports/<unit>/`. It exits with the suite's exit code and never masks a failure.
- `scripts/coverage/summarize.py` normalises Go coverprofiles, Cobertura XML, JaCoCo XML,
  LCOV and simplecov `.last_run.json` into one line-coverage table
  (`coverage-reports/summary.md` + `summary.json`).
- `make test-coverage` runs every unit (override with `COVERAGE_UNITS="api-gateway ..."`),
  always prints the aggregate table, and exits non-zero if any unit failed. Pass
  `BASELINE=coverage-baseline.json` to also fail on a coverage drop.
- CI: each per-service job uploads a `coverage-<unit>` artifact; the `coverage-summary` job
  aggregates them, publishes the table to the job summary and a PR comment, and enforces the
  **ratchet** against `coverage-baseline.json`.

### Declared decisions (so packages do not collide)

| Item | Decision |
|---|---|
| `--passWithNoTests` on `client-app` | **Deferred to WP-14.** WP-00 leaves it; removing it before real unit tests exist only turns CI red. |
| `\|\| true` on the `admin-dashboard` CI job | **Deferred to WP-16**, together with fixing the specs the flag currently hides. |
| `codecov.yml` | **Not used.** The repo has no Codecov integration, so the ratchet is implemented in-repo (`coverage-baseline.json` + `summarize.py --fail-on-drop`) rather than depending on an unconfigured external service. |
| `tests/contract` in `make test` | **Not included.** The contract suite needs a live service, so it would make `make test` unrunnable without Docker. It has its own target, `make test-contract`; **WP-19** wires it into CI against a composed stack. |
| `sonar.java.binaries=.` | Left as-is. The sources/tests overlap is fixed (test globs now excluded from `sonar.sources`), but resolving Java binaries needs a build step in the scan job, which is out of WP-00's scope. |
| Coverage instrumentation not yet wired | `analytics-service` (scoverage), `report-service` and `legacy-portal` (jacoco on their poms) — all owned by **WP-12**. They run their suites and appear in the table as "not instrumented". |

### Baseline

Baseline numbers are recorded in `coverage-baseline.json` at the repo root, generated from
CI's own run of this harness. Locally measured spot checks at the time of landing:

| Build unit | Line coverage |
|---|---:|
| `api-gateway` | 52.4% (215 / 410 statements) |
| `client-app` | 0.4% (7 / 1,586 lines) |

Acceptance checks performed: with a seeded failing test, `make test` and `make test-coverage`
both exit non-zero (and `test-coverage` still prints the full table, flagging the failing
unit); `summarize.py --fail-on-drop` exits non-zero on a simulated coverage regression.
