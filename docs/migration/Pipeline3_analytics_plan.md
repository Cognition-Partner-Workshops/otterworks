...
| **P3-D10** | `p3-analytics-daily`'s mapping compared whole gold tables against a one-day legacy baseline. Wave 2 seeds the 33 days before the run date into those same gold tables, because `user_activity_daily` reports over a 31-day window, so the comparison became 34 target days against 1 legacy day and failed on row counts. The conversion did not change; the shared table did. | Scope every object in the mapping to the run date on both sides (`summary_date = DATE '${run_date}'`), which is the day the mapping was always written about. No tolerance is widened, no field is dropped, and the unit's verdict still has to come from the official harness. | Approved after wave 2 |
| **P3-D11** | The cause of P3-D10 is general, not analytics-specific: `fixtures/seed_upstream_history.py` writes into gold tables that another unit reconciles, so one unit's fixture silently changes another unit's comparison. Nothing in the harness detects it — the failure surfaces as a row-count mismatch that looks like a broken conversion. | Every pipeline-3 mapping is now date- or batch-scoped, so a seeded neighbour cannot contaminate a unit's comparison. The stronger fix, which pipeline 3 does not implement, is to namespace fixture writes away from the reconciled tables entirely; whoever owns the fixture layer should do that before the next pipeline reuses it, because date scoping only works while every mapping remembers to apply it. | Approved after wave 2 |
