...
| **F-0.4** | `audit_archive_weekly.py` deletes with key `{event_id, timestamp}`. The `otterworks-audit-events` table's key schema is `id` alone. Every delete raises a validation error, and every delete is swallowed by a bare `except: pass`. The archive is written; **the source is never pruned**; `events_deleted_from_source` in the compliance report is 0. The same events are re-archived every week under a new key. | `audit_archive_weekly.py:140-165`, `scripts/localstack-init.sh:60-64`, `estate` |
...
| Clause | Contract | Evidence |
|---|---|---|
| C-5.1 | The cutoff is `midnight UTC of ds minus 90 days`, rendered `%Y-%m-%dT%H:%M:%SZ`-shaped via `isoformat() + "Z"`, and compared **as a string** with the item's `timestamp` attribute. Because both sides use the same zero-padded format, the string compare is chronologically correct; an item stored in any other timestamp format would compare wrongly. The comparison is strict `<`, so an event exactly on the cutoff is **not** archived. | `:52-74`, `source` |
...
| C-5.3 | The delete step does not work in this estate (F-0.4) and is swallowed. The archive is therefore append-only over an unshrinking source, and every weekly run re-archives every event past the cutoff. | `:139-165`, `estate` |
