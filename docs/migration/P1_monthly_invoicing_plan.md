...
4. PASS = tiers 1,2,3,5,7 `pass` with 0 unexplained row diffs; Tier 4 dual-run parity `pass` for every routine in the unit; Tier 6 `skipped` with reason `no_cdc` until the D10-12 CDC read path is populated (D10-10 is closed: Debezium connector `ow-tp-oracle-billing` is RUNNING, topics `ow_tp.oracle.OW_BILLING.<TABLE>`; Kafka is host-internal only, so the read path is a host-side sink writing change events to `/Volumes/ow_tp/bronze/landing/cdc/<TABLE>/`, parent-implemented, DEC-C4a); then Tier 6 = lag (source_ts vs applied_at, cdc_lag_max_s 60) and per-PK SCN ordering against that landing; `declared_source_rows` equals the census count at the pin (analysis §6 table); population per check = the whole table (no filters); baseline = Oracle at the pin, never fixture data.
