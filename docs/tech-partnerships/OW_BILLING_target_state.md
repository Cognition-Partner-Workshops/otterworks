...
| SQL warehouse identity | **FACT** — the workspace holds exactly one warehouse: `Serverless Starter Warehouse`, id `565cd2fd713738c4`, PRO serverless, auto-stop 10 minutes (live `GET /api/2.0/sql/warehouses`, 2026-08-28). Every unit uses that warehouse; a child that cannot find it stops and reports rather than creating one. Because auto-stop is 10 minutes, the first query of a run pays a cold start — a timeout there is not a capability failure. |
