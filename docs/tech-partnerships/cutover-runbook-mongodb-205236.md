...
| F.3.1a | **Quiescence barrier** (only if D.10 was reached): stop accepting traffic on the new deployment at the ingress, then scale the Atlas-connected `legacy-billing` deployment to **zero replicas** and wait for every pod to terminate (the `terminationGracePeriodSeconds` drain completes or kills in-flight requests — either way no request is mid-write afterwards); if a `nightly-dunning` run is in progress, wait for it to exit or terminate it, and confirm no execution is queued. Verify: (1) Atlas Metrics / `serverStatus.connections` shows **0** connections for the application database user; (2) two `db.serverStatus().opcounters` samples 60 s apart show **zero** delta in `insert`/`update`/`delete` on the cluster (or the Atlas Metrics opcounters chart flat at 0) and two `mongoexport` passes of `counters` (D.3 command) over the same interval have identical sha256. Only then proceed to F.3.5 | CUSTOMER (scale/terminate) + DEVIN (the two exports, read-only) |
