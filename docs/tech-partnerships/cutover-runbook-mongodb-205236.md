...
| D.3 | Target idle check: ns-scoped counts for the 18 mapped collections and 4 quarantine classes (`tools/guards.py`) equal section E.1. Then take the **watermark baseline snapshot** (F.3.5's baseline): for each of the 18 mapped collections and the 4 quarantine classes, `mongodump --uri "$MONGODB_ATLAS_URI" --db ow_tp_mongodb_205236 --collection <c> --query '{"ns":"mongo_205236"}' --gzip --archive=<dir>/<c>.bson.gz` (quarantine db likewise); write `manifest.json` with per-collection doc count + sha256 of each archive; counts must equal E.1/E.2 exactly. Commit the manifest to the evidence branch and the archives to the evidence branch (or, if over the repository size limit, to the customer-held evidence store, with the sha256 manifest on the branch) | DEVIN (read-only on target, evidence-writing) | Any count diff → abort. `mongodump` is a read; nothing is written to the target. The snapshot carries full documents, so F.3.5 can express deletes (in baseline, absent now), in-place updates (same `_id`, different canonical form) and inserts |
...
| F.3.5 | If D.10 was reached (run **before** F.3.4, while legacy is still frozen and after F.3.1 has stopped the new stack's writers): produce the **full change inventory** since the watermark — inserts, updates and deletes. Procedure: (1) `mongodump` every one of the 18 mapped collections ns-scoped, exactly as in D.3, to `<dir>/now/`; (2) `bsondump` both the D.3 **watermark baseline** (17 non-audit collections) / the D.9 **post-verification baseline** (`billing_audit_log`, `counters`) and the new dump to canonical JSON (`.migration/canonicalization.json` rules via the harness canonicaliser, sorted keys) keyed by `_id`; (3) classify per `_id`: in baseline only → **delete**, in new dump only → **insert**, in both with different canonical form → **update** (emit before/after); (4) write `inventory.jsonl` + per-collection counts to the evidence branch. Baselines are full documents, so in-place updates of pre-existing `subscriptions`, `subscriptions_history`, `rating_periods`, `billing_invoices`, `credit_notes`, `tenants` and `counters` documents are captured; an `_id`-only export would not be. The inventory is what the customer replays into Oracle or discards. This procedure is exercised in the F.4 dry run (one synthetic update, one insert, one delete must appear in the inventory) | DEVIN (read-only on target, evidence-writing) |
...
(e.g. a plan change on a staging copy), one synthetic insert and one synthetic delete to prove F.3.5's baseline diff
reports all three classes, and attach the transcript to the STOP C request.
