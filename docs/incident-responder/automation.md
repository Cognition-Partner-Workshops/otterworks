...
- **Branch from the alert's `branch` label, never from the namespace** — `scripts/deploy-tenant.sh` stamps the branch a tenant deploys onto its alert rules (`monitoring.rules.extraLabels.branch`); `workshop-<id>` and `demo-<id>` share a tenant id and a tenant can track `main`, so a namespace-derived guess can land the PR on the wrong base. No label and a tenant namespace means stop, not guess.
