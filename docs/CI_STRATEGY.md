...
| Job | Tool | Purpose |
|-----|------|---------|
| `dependency-scan` | Trivy v0.36.0 | Scans filesystem for CRITICAL/HIGH CVEs. Skips `report-service` because it deliberately keeps the commons-text 1.9 CVE fixture for the remediation lab. Uses `.trivyignore` for acknowledged CVEs. |
...
3. **Report service isolation** — Report service is skipped by Trivy because it deliberately keeps the commons-text 1.9 CVE fixture for the remediation lab.
